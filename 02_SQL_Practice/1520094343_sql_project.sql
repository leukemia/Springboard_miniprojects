...
SELECT DISTINCT CONCAT (Firstname, ' ', surname) AS mem_name,
...
SELECT CONCAT (firstname, ' ', surname) AS mem_name,
	   Facilities.name AS fac_name,
...
SELECT *
FROM ( SELECT CONCAT (firstname, ' ', surname) AS mem_name,
	   Facilities.name AS fac_name,
	   CASE WHEN Members.memid = 0 THEN (Bookings.slots * Facilities.guestcost)
	   ELSE (Bookings.slots * Facilities.membercost)
	   END AS total_cost
...
FROM (
		SELECT Facilities.name,
			   ...
			   SUM(CASE WHEN Bookings.memid = 0 THEN Bookings.slots * Facilities.guestcost
						ELSE Bookings.slots * membercost END) AS revenue
			   FROM Facilities 
			   INNER JOIN Bookings
